...
def rot6d_to_axis_angle(x):
    ...
    rot_mat = torch.cat([rot_mat, to_device(torch.zeros((batch_size, 3, 1)).float(), rot_mat.device)], 2)  # 3x4 rotation matrix
...
def soft_argmax_2d(heatmap2d):
    ...
    accu_x = accu_x * to_device(torch.arange(width).float(), heatmap2d.device)[None, None, :]
    accu_y = accu_y * to_device(torch.arange(height).float(), heatmap2d.device)[None, None, :]
...
def soft_argmax_3d(heatmap3d):
    ...
    accu_x = accu_x * to_device(torch.arange(width).float(), heatmap3d.device)[None, None, :]
    accu_y = accu_y * to_device(torch.arange(height).float(), heatmap3d.device)[None, None, :]
    accu_z = accu_z * to_device(torch.arange(depth).float(), heatmap3d.device)[None, None, :]
