"""
SMPLest-X model with VPoser integration
Extended version that supports VPoser-based pose priors for improved pose estimation
"""
...
class ModelWithVPoser(nn.Module):
    """
    SMPLest-X model enhanced with VPoser pose prior
    Supports both traditional pose prediction and VPoser latent code prediction
    """
    
    def __init__(self, config, encoder, decoder):
        super(ModelWithVPoser, self).__init__()
        ...
        # VPoser integration
        self.use_vposer = getattr(config.model, 'use_vposer', False)
        self.vposer_wrapper = None
        self.vposer_loss = None
        
        if self.use_vposer:
            self._initialize_vposer()
        ...
        if self.use_vposer and self.vposer_wrapper:
            print("VPoser integration enabled")
        else:
            print("VPoser integration disabled or failed")

    def _initialize_vposer(self):
        ...
        try:
            ...
            if self.vposer_wrapper:
                ...
                logging.info("VPoser integration initialized successfully")
            else:
                logging.warning("VPoser initialization failed - continuing without pose prior")
                self.use_vposer = False
                
        except Exception as e:
            logging.warning(f"VPoser initialization error: {e}")
            self.use_vposer = False
    ...
    def process_body_pose_with_vposer(self, body_pose_pred, mode='train'):
        """
        Process body pose predictions with VPoser regularization
        
        Args:
            body_pose_pred: Predicted body pose [batch_size, 63]
            mode: 'train' or 'test'
            
        Returns:
            regularized_pose: VPoser-regularized body pose
            vposer_losses: Dictionary of VPoser losses (training only)
        """
        if not self.use_vposer or not self.vposer_wrapper:
            return body_pose_pred, {}
            
        if mode == 'train':
            # Compute VPoser losses for training
            vposer_losses = self.vposer_loss(
                body_pose_pred, 
                pose_prior_weight=getattr(self.cfg.train, 'vposer_prior_weight', 1.0),
                reconstruction_weight=getattr(self.cfg.train, 'vposer_recon_weight', 10.0)
            )
            
            # Apply light regularization during training
            regularization_strength = getattr(self.cfg.train, 'vposer_regularization', 0.1)
            regularized_pose = self.vposer_wrapper.regularize_pose(
                body_pose_pred, alpha=regularization_strength
            )
            
            return regularized_pose, vposer_losses
        else:
            # Apply stronger regularization during inference
            regularization_strength = getattr(self.cfg.test, 'vposer_regularization', 0.3)
            regularized_pose = self.vposer_wrapper.regularize_pose(
                body_pose_pred, alpha=regularization_strength
            )
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        # VPoser processing for body pose
        body_pose_aa_regularized, vposer_losses = self.process_body_pose_with_vposer(body_pose_aa, mode)
        ...
        body_pose_rotmat = batch_rodrigues(body_pose_aa_regularized.reshape(-1, 3)).reshape(body_pose_aa_regularized.shape[0], -1)  
        ...
        # kps2d, kps3d and mesh output - use regularized body pose
        joint_proj, joint_cam, joint_cam_wo_ra, mesh_cam, pred_root_cam = self.get_coord(root_pose_aa, body_pose_aa_regularized, lhand_pose_aa, 
        ...
        if mode == 'train':
            ...
            ### VPoser losses ###
            if self.use_vposer and vposer_losses:
                for vposer_loss_name, vposer_loss_value in vposer_losses.items():
                    loss[vposer_loss_name] = vposer_loss_value
            ...
            out['smplx_body_pose'] = body_pose_aa_regularized  # Use regularized pose
...
def get_model_with_vposer(cfg, mode):
    """
    Factory function to create SMPLest-X model with VPoser integration
    """
    ...
    model = ModelWithVPoser(cfg, encoder, decoder)
    return model